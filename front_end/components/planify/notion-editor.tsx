...
import { Bold, Italic, Underline as UnderlineIcon, Strikethrough, Heading1, Heading2, Code, Link2 } from "lucide-react"
...
export default function NotionEditor({
  ...
  const savePage = useCallback(() => {
    ...
    const pageIndex = pages.findIndex((page: any) => page.id === id)

    if (pageIndex !== -1) {
      pages[pageIndex] = { ...pages[pageIndex], title, content }
    } else {
      pages.push({ id, title, content, type: "editor" })
    }
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto text-white">
      <Input
        ...
        className="text-3xl font-bold mb-4 w-full bg-transparent border-none outline-none placeholder-gray-400"
        ...
        {editor && (
          ...
          >
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("bold") && "bg-gray-700")}
            ...
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("italic") && "bg-gray-700")}
  ...
  variant="ghost"
  size="sm"
  className={cn("p-1 h-8 w-8", editor.isActive("link") && "bg-gray-700")}
  onClick={() => {
...
>
  <Link2 className="h-4 w-4 text-white" />
            ...
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("underline") && "bg-gray-700")}
            ...
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("strike") && "bg-gray-700")}
            ...
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("code") && "bg-gray-700")}
            ...
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("heading", { level: 1 }) && "bg-gray-700")}
            ...
            <Button
              variant="ghost"
              size="sm"
              className={cn("p-1 h-8 w-8", editor.isActive("heading", { level: 2 }) && "bg-gray-700")}
        ...
        <EditorContent
          editor={editor}
          className="prose prose-sm sm:prose-base lg:prose-lg max-w-none focus:outline-none text-white"
        />
      </div>
    </div>
  )
